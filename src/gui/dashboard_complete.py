...
class MetricCard(ctk.CTkFrame):
    def __init__(self, parent, title="", value="0", icon="📊", color=COLORS['primary'], **kwargs):
        ...
        icon_frame = ctk.CTkFrame(self, fg_color=color, width=50, height=50, corner_radius=10)
...
class CompleteDashboard:
    def __init__(self):
        self.scheduler = CompleteScheduler(4)
        self.setup_gui()
        self.update_ui()
        
    def setup_gui(self):
        self.root = ctk.CTk()
        self.root.title("Multi-Core Load Balancing Scheduler - Complete Edition")
        ...
        # SECTION 1: CPU Cores (Top)
        self.setup_cores_section()
        
        # SECTION 2: Utilization History Chart (Dedicated Large Section)
        self.setup_chart_section()
        
        # SECTION 3: Performance Metrics (Left) + Process Queue (Right)
        self.setup_metrics_and_queue_section()
    ...
    def setup_cores_section(self):
        ...
        cores_container.pack(fill="x", pady=(0, 20))
    ...
    def setup_chart_section(self):
        ...
        ctk.CTkLabel(
            header_frame, text="📈 UTILIZATION HISTORY CHART (Real-time)",
            font=ctk.CTkFont(family="Segoe UI", size=16, weight="bold"),
            text_color=COLORS['primary']
        ).pack(side="left")
        ...
        legend_colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4']
        legend_names = ['Core 0', 'Core 1', 'Core 2', 'Core 3']
        for color, name in zip(legend_colors, legend_names):
            ...
            color_box = ctk.CTkFrame(item, width=15, height=15, fg_color=color, corner_radius=3)
            color_box.pack(side="left", padx=(0, 5))
            color_box.pack_propagate(False)
            
            ctk.CTkLabel(item, text=name, font=ctk.CTkFont(size=10), text_color=COLORS['text_secondary']).pack(side="left")
        ...
        # Create large figure (800x400)
        self.fig = Figure(figsize=(14, 5), dpi=100, facecolor=COLORS['bg_dark'])
        self.ax = self.fig.add_subplot(111)
        self.ax.set_facecolor('#1a1f3a')
        self.ax.set_title("CPU Core Utilization Over Time", color='white', fontsize=14, fontweight='bold', pad=15)
        ...
        # Data storage (60 points = 30 seconds history)
        self.chart_data = {0: deque(maxlen=60), 1: deque(maxlen=60), 2: deque(maxlen=60), 3: deque(maxlen=60)}
        self.time_data = deque(maxlen=60)
    
    def setup_metrics_and_queue_section(self):
        """Split section: Performance Metrics (Left) and Process Queue (Right)"""
        bottom_container = ctk.CTkFrame(self.main_container, fg_color="transparent")
        bottom_container.pack(fill="both", expand=True)
        
        # LEFT: Performance Metrics Section
        metrics_container = ctk.CTkFrame(bottom_container, fg_color=COLORS['bg_card'], corner_radius=15)
        metrics_container.pack(side="left", fill="both", expand=True, padx=(0, 8))
        
        ctk.CTkLabel(
            metrics_container, text="📊 PERFORMANCE METRICS",
            font=ctk.CTkFont(family="Segoe UI", size=15, weight="bold"),
            text_color=COLORS['primary']
        ).pack(pady=(12, 8))
        
        # Metrics grid (3x3 for 9 metrics)
        ...
        metrics = [
            ...
            ("Avg Turnaround", "0s", "🔄", COLORS['success'])
        ]
        ...
        # RIGHT: Process Queue Section
        queue_container = ctk.CTkFrame(bottom_container, fg_color=COLORS['bg_card'], corner_radius=15)
        queue_container.pack(side="right", fill="both", expand=True, padx=(8, 0))
        ...
        tree_frame = ctk.CTkFrame(queue_container, fg_color="transparent")
        ...
        # Additional info
        info_frame = ctk.CTkFrame(queue_container, fg_color="transparent")
        info_frame.pack(fill="x", padx=15, pady=(0, 12))
        
        self.waiting_label = ctk.CTkLabel(
            info_frame, text="⏱️ Avg Waiting: 0.00s",
            font=ctk.CTkFont(family="Segoe UI", size=11),
            text_color=COLORS['text_secondary']
        )
        ...
        self.turnaround_label = ctk.CTkLabel(
            info_frame, text="🔄 Avg Turnaround: 0.00s",
            font=ctk.CTkFont(family="Segoe UI", size=11),
            text_color=COLORS['text_secondary']
        )
    ...
    def start_scheduler(self):
        ...
        self.status_label.configure(text="▶ Scheduler Running - Load Balancing Active")
    ...
    def add_process(self):
        ...
        self.status_label.configure(text=f"✅ Added Process {process['name']}")
    ...
    def clear_all(self):
        ...
        self.start_btn.configure(state="normal")
        self.stop_btn.configure(state="disabled")
        self.status_label.configure(text="🔄 System Reset Complete")
    
    def change_policy(self, choice):
        self.scheduler.change_policy(choice)
        self.status_label.configure(text=f"📋 Scheduling Policy Changed to: {choice}")
    ...
    def update_ui(self):
        try:
            ...
            self.metric_cards["Avg Turnaround"].set_value(f"{state['avg_turnaround_time']:.2f}s")
...
if __name__ == "__main__":
    print("""
    ╔═══════════════════════════════════════════════════════════════════╗
    ║     Multi-Core Load Balancing Scheduler - Complete Edition       ║
    ║                         Version 4.0.0                           ║
    ╚═══════════════════════════════════════════════════════════════════╝
    """)
    app = CompleteDashboard()
    app.run()
